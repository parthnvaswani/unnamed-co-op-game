...
load_img = pygame.image.load(images["load_btn"])
...
def draw_grid():
    for c in range(21):
        # vertical lines
        pygame.draw.line(
            screen, white, (c * tile_size, 0), (c *
                                                tile_size, screen_height - margin)
        )
        # horizontal lines
        pygame.draw.line(
        ...
        for col in range(20):
            if world_data[row][col] > 0:
                ...
                if world_data[row][col] == 7:
                    # coin
                    img = pygame.transform.scale(
                        coin_img, (tile_size / 1.5, tile_size / 1.5)
                    )
                    screen.blit(
                        img,
                        (
                ...
                if world_data[row][col] == 8:
                    # exit
                    img = pygame.transform.scale(
                        exit_img, (tile_size, int(tile_size * 1.3))
                    )
                    screen.blit(
                        img, (col * tile_size, row *
...
while run:
    ...
    draw_text(
        "Press UP or DOWN to change level", font, white, tile_size, screen_height - 40
    )
